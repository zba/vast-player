...
function defaults(/*...objects*/) {
    ...
    for (index = 0; index < length; index++) {
        ...
        for (prop in object) {
            ...
            if (typeof value === 'object' && !Array.prototype.isPrototypeOf(value)) {
...
function VASTPlayer(container, config) {
    ...
    this.on(EVENTS.AdClickThru, function onAdClickThru(url, id, playerHandles) {
        ...
        if (
          self.config.clickThroughHandler && typeof self.config.clickThroughHandler === 'function'
        ) {
          self.config.clickThroughHandler(clickThrough);
          return;
        }
    ...
    return VAST.fetch(uri, config).then(function loadPlayer(vast) {
        var config = (function() {
            var jsVPAIDFiles = vast.filter('ads[0].creatives[0].mediaFiles', function(mediaFile) {
                return !self.config.prohibitVPAID && !self.config.prohibitJsVPAID && (
            ...
            var swfVPAIDFiles = vast.filter('ads[0].creatives[0].mediaFiles', function(mediaFile) {
                return !self.config.prohibitVPAID && !self.config.prohibitSwfVPAID && mediaFile.type === MIME.FLASH && mediaFile.apiFramework === 'VPAID';
            });
            ...
            if (jsVPAIDFiles.length > 0) {
                ...
                if (self.config.keepADS) {
                  playerData.player.keepADS = true;
                }
                if (self.config.revertToStart) {
                  playerData.player.revertToStart = true;
                }
                if (self.config.revertToLast) {
                  playerData.player.revertToLast = true;
                }
                return playerData;
            ...
            playerData = {
              player: new HTMLVideoPlayer(self.container, self.config.videoOptions),
              mediaFiles: files
            };
            if (self.config.keepADS) {
              playerData.player.keepADS = true;
